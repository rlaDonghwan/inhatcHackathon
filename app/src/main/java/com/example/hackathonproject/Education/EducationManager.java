...
public class EducationManager {
    ...
    public boolean addEducationPost(String title, String category, String content, String location, int userId) {
        // 현재 한국 표준시(KST) 시간을 가져옴
        ZonedDateTime kstTime = ZonedDateTime.now(ZoneId.of("Asia/Seoul"));
        return educationDAO.insertEducationPost(title, category, content, location, userId, kstTime);
    }
