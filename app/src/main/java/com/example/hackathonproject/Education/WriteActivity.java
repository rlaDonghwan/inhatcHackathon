...
public class WriteActivity extends AppCompatActivity {
    ...
    private int postId = -1; // 수정 시 사용할 게시글 ID
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent.hasExtra("postId")) {
            postId = intent.getIntExtra("postId", -1);  // 게시글 ID 가져오기
    ...
    private void submitEducation() {
        ...
        // 비동기 작업으로 게시글 등록
        new SubmitEducationTask().execute(title, category, description, "서울");  // 위치는 임의로 "서울"로 설정
    }
    //-----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    private void updateEducationPost() {
        ...
        // 비동기 작업으로 게시글 수정
        new UpdateEducationTask().execute(postId, title, category, description, "서울");  // 위치는 임의로 "서울"로 설정
    }
    //-----------------------------------------------------------------------------------------------------------------------------------------------

    // 비동기 작업으로 새 게시글을 데이터베이스에 등록하는 클래스
    private class SubmitEducationTask extends AsyncTask<String, Void, Boolean> {
        ...
        protected Boolean doInBackground(String... params) {
            ...
            try {
                ...
                // 데이터를 데이터베이스에 삽입
                return educationDAO.insertEducationPost(title, category, description, location, userId, kstTime);
            } catch (Exception e) {
    ...
    private class UpdateEducationTask extends AsyncTask<Object, Void, Boolean> {
        ...
        protected Boolean doInBackground(Object... params) {
            int postId = (int) params[0];  // 게시글 ID 가져오기
            ...
            try {
                // 데이터를 데이터베이스에서 업데이트
                return educationDAO.updateEducationPost(postId, title, category, description, location, userId);
            } catch (Exception e) {
        ...
        protected void onPostExecute(Boolean success) {
            if (success) {
                Toast.makeText(WriteActivity.this, "게시글이 성공적으로 수정되었습니다.", Toast.LENGTH_SHORT).show();  // 성공 메시지 표시
                setResult(RESULT_OK);  // 결과 설정
                finish();  // 액티비티 종료
            } else {
                Toast.makeText(WriteActivity.this, "게시글 수정에 실패하였습니다. 다시 시도해 주세요.", Toast.LENGTH_SHORT).show();  // 실패 메시지 표시
            }
        }
    }
    //-----------------------------------------------------------------------------------------------------------------------------------------------
