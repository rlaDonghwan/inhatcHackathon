...
public class SeminarActivity extends AppCompatActivity {
    ...
    private SeminarAdapter adapter;  // SeminarAdapter 변수 선언 (RecyclerView에 데이터를 연결할 때 사용)
    private List<SeminarPost> seminarPostList = new ArrayList<>();  // 세미나 게시글 리스트를 저장할 리스트 변수 선언
    private SwipeRefreshLayout swipeRefreshLayout;  // SwipeRefreshLayout 변수 선언 (새로고침 기능을 구현할 때 사용)
    private SeminarManager seminarManager;  // SeminarManager 변수 선언 (세미나 게시글 데이터를 관리할 때 사용)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        seminarManager = new SeminarManager(new SeminarDAO());  // SeminarManager 객체 초기화
        ...
        adapter = new SeminarAdapter(seminarPostList);  // SeminarAdapter 초기화 (데이터를 연결하기 위해)
        ...
        fab.setOnClickListener(v -> {
            Intent intent = new Intent(SeminarActivity.this, SeminarWriteActivity.class);  // 새 게시글 작성 화면으로 이동하는 인텐트 설정
        ...
        adapter.setOnItemClickListener(new SeminarAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                SeminarPost post = seminarPostList.get(position);  // 클릭된 게시글의 정보를 가져옴
                Intent intent = new Intent(SeminarActivity.this, SeminarContentView.class);  // 세미나 상세보기 화면으로 이동하는 인텐트 설정
