...
public class LectureActivity extends AppCompatActivity {
    private RecyclerView recyclerView;  // RecyclerView 변수 선언 (강연 게시글 리스트를 보여줄 때 사용)
    private LectureAdapter adapter;  // LectureAdapter 변수 선언 (RecyclerView에 데이터를 연결할 때 사용)
    private List<LecturePost> lecturePostList = new ArrayList<>();  // 강연 게시글 리스트를 저장할 리스트 변수 선언
    private SwipeRefreshLayout swipeRefreshLayout;  // SwipeRefreshLayout 변수 선언 (새로고침 기능을 구현할 때 사용)
    private LectureManager lectureManager;  // LectureManager 변수 선언 (강연 게시글 데이터를 관리할 때 사용)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lectureManager = new LectureManager(new LectureDAO());  // LectureManager 객체 초기화
        ...
        adapter = new LectureAdapter(lecturePostList);  // LectureAdapter 초기화 (데이터를 연결하기 위해)
        recyclerView.setAdapter(adapter);  // RecyclerView에 어댑터 설정

        swipeRefreshLayout.setOnRefreshListener(this::loadLecturePosts);  // 새로고침 리스너 설정 (사용자가 새로고침할 때 호출될 메서드)
        ...
        loadLecturePosts();  // 강연 게시글 로드 메서드 호출
        ...
        // LecturePost(강연 게시글) 클릭 시 이벤트 처리
        adapter.setOnItemClickListener(new LectureAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
                LecturePost post = lecturePostList.get(position);  // 클릭된 게시글의 정보를 가져옴
                Intent intent = new Intent(LectureActivity.this, LectureContentView.class);  // 강연 상세보기 화면으로 이동하는 인텐트 설정
    ...
    private class LoadPostsTask extends AsyncTask<Void, Void, List<LecturePost>> {
        ...
        protected List<LecturePost> doInBackground(Void... voids) {
            return lectureManager.getAllLecturePosts();  // LectureManager를 통해 모든 강연 게시글 데이터를 가져옴
        }

        @Override
        protected void onPostExecute(List<LecturePost> posts) {
            lecturePostList = posts;  // 가져온 강연 게시글 데이터를 리스트에 저장
            adapter.updateData(lecturePostList);  // 어댑터에 새로운 데이터 설정
